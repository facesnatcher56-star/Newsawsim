...
func _init():
	...
	var scene = load("res://game/transport/conveyors/chain_trough_conveyor.tscn")
	if not scene:
		file.store_line("Failed to load res://game/transport/conveyors/chain_trough_conveyor.tscn")
		quit(1)
		return
