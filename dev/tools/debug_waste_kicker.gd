...
func _init():
	...
	var scene = load("res://game/transport/conveyors/waste_conveyor_kicker.tscn")
	if not scene:
		file.store_line("Failed to load res://game/transport/conveyors/waste_conveyor_kicker.tscn")
		quit(1)
		return
