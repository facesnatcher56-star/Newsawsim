...
@export_range(2.5, 6.0, 0.1) var bin_depth: float = 4.2:
...
func _build_overhead_chain_lugs() -> void:
	...
	for side in [-1.0, 1.0]:
		for i in range(lug_count):
			...
			lug.name = "DragLug_Z%d_%d" % [int(side), i]
			lug.sync_to_physics = true

			var lug_mesh := CSGBox3D.new()
			lug_mesh.name = "LugTooth"
			lug_mesh.size = Vector3(0.06, 0.12, 0.08)
			lug_mesh.position = Vector3(0.0, -0.06, side * 0.8)
...
func _on_infeed_body_entered(body: Node3D) -> void:
	...
	# Measure board length along Z axis or extent bounding box
	var board_length: float = 4.0  # default estimate
...
func _physics_process(delta: float) -> void:
	...
	_chain_offset = fmod(_chain_offset + conveyor_speed * delta, num_bins * bin_width + 1.0)

	# Process active tracked boards and pneumatic drop gates
	...
	while i >= 0:
		...
		# Check if board has reached target bin bay
		if local_pos.x >= target_x - 0.4 and local_pos.x <= target_x + 0.6:
			# Trigger pneumatic drop gate opening
			_target_gate_angles[tracking.target_bin] = 0.55  # ~31 degrees down
			_set_bay_status_led(tracking.target_bin, "Yellow")

			# Apply slight downward push to assist gravity drop
			tracking.board.apply_central_impulse(Vector3(0.5, -2.0, 0.0))
			tracking.active = false
		i -= 1

	# Smoothly actuate pneumatic drop gates and piston rods
	for b in range(num_bins):
		...
		if not is_equal_approx(cur_angle, target_angle):
			...
			if b < _piston_nodes.size():
				var extension: float = (gate.rotation.z / 0.55) * 0.22
				for piston in _piston_nodes[b]:
					if is_instance_valid(piston):
						piston.position.y = -0.30 - extension

		# Reset gate after board has dropped
		if is_equal_approx(gate.rotation.z, 0.55):
