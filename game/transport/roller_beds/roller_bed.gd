...
const RollerBedSweepChainScript = preload("res://game/transport/roller_beds/roller_bed_sweep_chain.gd")
...
var _sweep_chain = RollerBedSweepChainScript.new()
...
func _physics_process(delta: float) -> void:
	...
	_ensure_sweep_chain()
	_sweep_chain.physics_process(delta)
...
func _ensure_sweep_chain() -> void:
	if _sweep_chain == null:
		_sweep_chain = RollerBedSweepChainScript.new()
	_sweep_chain.setup(self)


func _rebuild_sweep_chains(travel: Vector3, _roller_axis: Vector3, local_up: Vector3, radius: float) -> void:
	_ensure_sweep_chain()
	_sweep_chain.rebuild(travel, local_up, radius)


func trigger_sweep() -> void:
	_ensure_sweep_chain()
	_sweep_chain.trigger()
